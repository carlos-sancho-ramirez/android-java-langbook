...
abstract class LangbookDatabaseChecker<LanguageId extends LanguageIdInterface, AlphabetId extends AlphabetIdInterface, SymbolArrayId extends SymbolArrayIdInterface, CorrelationId extends CorrelationIdInterface, CorrelationArrayId extends CorrelationArrayIdInterface, AcceptationId extends AcceptationIdInterface> implements LangbookChecker<LanguageId, AlphabetId, SymbolArrayId, CorrelationId, AcceptationId> {
    ...
    LangbookDatabaseChecker(Database db, IntSetter<LanguageId> languageIdManager, IntSetter<AlphabetId> alphabetIdManager, IntSetter<SymbolArrayId> symbolArrayIdManager, IntSetter<CorrelationId> correlationIdSetter, IntSetter<CorrelationArrayId> correlationArrayIdSetter, IntSetter<AcceptationId> acceptationIdSetter) {
    ...
    private Integer selectOptionalFirstIntColumn(DbQuery query) {
        Integer result = null;
    ...
    private ImmutableIntSet readBunchesFromSetOfBunchSets(ImmutableIntSet bunchSets) {
        final ImmutableIntSet.Builder builder = new ImmutableIntSetCreator();
        for (int bunchSet : bunchSets) {
            for (int bunch : getBunchSet(bunchSet)) {
                builder.add(bunch);
            }
        }
    ...
    public ImmutableIntKeyMap<String> readAllMatchingBunches(ImmutableCorrelation<AlphabetId> texts, AlphabetId preferredAlphabet) {
        ...
        final ImmutableIntSet bunches = readBunchesFromSetOfBunchSets(validBunchSetsBuilder.build());
        final ImmutableIntKeyMap.Builder<String> builder = new ImmutableIntKeyMap.Builder<>();
        for (int bunch : bunches) {
            builder.put(bunch, readConceptText(bunch, preferredAlphabet));
        }
    ...
    public ImmutableIntKeyMap<ImmutableSet<QuestionFieldDetails<AlphabetId>>> readQuizSelectorEntriesForBunch(int bunch) {
        ...
        try (DbResult dbResult = _db.select(query)) {
            while (dbResult.hasNext()) {
                ...
                final QuestionFieldDetails<AlphabetId> field = new QuestionFieldDetails<>(_alphabetIdSetter.getKeyFromDbValue(row.get(1)), row.get(2).toInt(), row.get(3).toInt());
    ...
    private AcceptationOrigin<AcceptationId> readOriginalAcceptation(AcceptationId acceptation, AlphabetId preferredAlphabet) {
    ...
    private AppliedRule<AcceptationId> readRulePreferredTextByAgent(int agentId, AlphabetId preferredAlphabet) {
        final LangbookDbSchema.AgentsTable agents = LangbookDbSchema.Tables.agents;
        final LangbookDbSchema.AcceptationsTable acceptations = LangbookDbSchema.Tables.acceptations;
        final LangbookDbSchema.StringQueriesTable strings = LangbookDbSchema.Tables.stringQueries;
        final int accOffset = agents.columns().size();
    ...
    private DerivedAcceptationsReaderResult<AcceptationId> readDerivedAcceptations(AcceptationId acceptation, AlphabetId preferredAlphabet) {
        final LangbookDbSchema.RuledAcceptationsTable ruledAcceptations = LangbookDbSchema.Tables.ruledAcceptations;
        final LangbookDbSchema.StringQueriesTable strings = LangbookDbSchema.Tables.stringQueries;
        final LangbookDbSchema.AgentsTable agents = LangbookDbSchema.Tables.agents;
        ...
        final ImmutableIntKeyMap<String> ruleTexts = agentRules.toSet().assign(rule -> readConceptText(rule, preferredAlphabet)).toImmutable();
    ...
    private ImmutableIntSet readAgentsWhereAcceptationIsTarget(AcceptationId staticAcceptation) {
        final LangbookDbSchema.AcceptationsTable acceptations = LangbookDbSchema.Tables.acceptations;
    ...
    private ImmutableIntSet readAgentsWhereAcceptationIsSource(AcceptationId staticAcceptation) {
        final LangbookDbSchema.AcceptationsTable acceptations = LangbookDbSchema.Tables.acceptations;
        final LangbookDbSchema.AgentsTable agents = LangbookDbSchema.Tables.agents;
        final LangbookDbSchema.BunchSetsTable bunchSets = LangbookDbSchema.Tables.bunchSets;
    ...
    private ImmutableIntSet readAgentsWhereAcceptationIsRule(AcceptationId staticAcceptation) {
        final LangbookDbSchema.AcceptationsTable acceptations = LangbookDbSchema.Tables.acceptations;
        final LangbookDbSchema.AgentsTable agents = LangbookDbSchema.Tables.agents;
    ...
    private ImmutableIntSet readAgentsWhereAcceptationIsProcessed(AcceptationId acceptation) {
        final LangbookDbSchema.BunchAcceptationsTable bunchAcceptations = LangbookDbSchema.Tables.bunchAcceptations;
        ...
        return _db.select(query).mapToInt(row -> row.get(0).toInt()).filter(agentId -> agentId != 0).toSet().toImmutable();
    }
     ...
     * @return a map whose keys are agent identifier and values are flags. Flags should match the values at {@link sword.langbook3.android.models.AcceptationDetailsModel.InvolvedAgentResultFlags}
     */
    private ImmutableIntPairMap readAcceptationInvolvedAgents(AcceptationId staticAcceptation) {
        final MutableIntPairMap flags = MutableIntPairMap.empty();

        for (int agentId : readAgentsWhereAcceptationIsTarget(staticAcceptation)) {
            flags.put(agentId, flags.get(agentId, 0) | AcceptationDetailsModel.InvolvedAgentResultFlags.target);
        }

        for (int agentId : readAgentsWhereAcceptationIsSource(staticAcceptation)) {
            flags.put(agentId, flags.get(agentId, 0) | AcceptationDetailsModel.InvolvedAgentResultFlags.source);
        }

        // TODO: Diff not implemented as right now it is impossible

        for (int agentId : readAgentsWhereAcceptationIsRule(staticAcceptation)) {
            flags.put(agentId, flags.get(agentId, 0) | AcceptationDetailsModel.InvolvedAgentResultFlags.rule);
        }

        for (int agentId : readAgentsWhereAcceptationIsProcessed(staticAcceptation)) {
            flags.put(agentId, flags.get(agentId, 0) | AcceptationDetailsModel.InvolvedAgentResultFlags.processed);
        }
    ...
    public AcceptationDetailsModel<LanguageId, AlphabetId, CorrelationId, AcceptationId> getAcceptationsDetails(AcceptationId staticAcceptation, AlphabetId preferredAlphabet) {
        ...
        final AcceptationOrigin<AcceptationId> origin = readOriginalAcceptation(staticAcceptation, preferredAlphabet);
        ...
        if (appliedAgentId != 0) {
            final AppliedRule<AcceptationId> appliedRule = readRulePreferredTextByAgent(appliedAgentId, preferredAlphabet);
        ...
        final DerivedAcceptationsReaderResult<AcceptationId> morphologyResults = readDerivedAcceptations(staticAcceptation, preferredAlphabet);
        ...
        final ImmutableIntPairMap involvedAgents = readAcceptationInvolvedAgents(staticAcceptation);
        ...
        final ImmutableIntKeyMap<String> sampleSentences = getSampleSentences(staticAcceptation);
    ...
    public QuizDetails<AlphabetId> getQuizDetails(int quizId) {
        ...
        try (DbResult result = _db.select(query)) {
            while (result.hasNext()) {
                ...
                builder.add(new QuestionFieldDetails<>(_alphabetIdSetter.getKeyFromDbValue(row.get(1)), row.get(2).toInt(), row.get(3).toInt()));
            }
        }
    ...
    private String readApplyRuleText(AcceptationId acceptation, QuestionFieldDetails<AlphabetId> field) {
        final LangbookDbSchema.AgentsTable agents = LangbookDbSchema.Tables.agents;
        final LangbookDbSchema.RuledAcceptationsTable ruledAcceptations = LangbookDbSchema.Tables.ruledAcceptations;
        final LangbookDbSchema.StringQueriesTable strings = LangbookDbSchema.Tables.stringQueries;
    ...
    public AgentDetails<AlphabetId> getAgentDetails(int agentId) {
        ...
        final ImmutableIntSet targetBunches = getBunchSet(register.targetBunchSetId);
        final ImmutableIntSet sourceBunches = getBunchSet(register.sourceBunchSetId);
    ...
    public MorphologyReaderResult<AcceptationId> readMorphologiesFromAcceptation(AcceptationId acceptation, AlphabetId preferredAlphabet) {
        ...
        final ImmutableIntKeyMap<String> ruleTexts = rules.assign(rule -> readConceptText(rule, preferredAlphabet));
    ...
    @Override
    public ImmutableSet<SentenceSpan<AcceptationId>> getSentenceSpans(int sentenceId) {
    ...
    private ImmutableIntSet findSentenceIdsMatchingMeaning(int sentenceId) {
        final LangbookDbSchema.SentencesTable table = LangbookDbSchema.Tables.sentences;
        final int offset = table.columns().size();
        final DbQuery query = new DbQuery.Builder(table)
    ...
    public SentenceDetailsModel<AcceptationId> getSentenceDetails(int sentenceId) {
        ...
        final ImmutableIntKeyMap<String> sameMeaningSentences = findSentenceIdsMatchingMeaning(sentenceId).assign(this::getSentenceText);
        return new SentenceDetailsModel<>(conceptAndText.concept, conceptAndText.text, spans, sameMeaningSentences);
    }

    ImmutableIntSet findCorrelationsByLanguage(LanguageId language) {
        final LangbookDbSchema.CorrelationsTable correlations = LangbookDbSchema.Tables.correlations;
        final LangbookDbSchema.AlphabetsTable alphabets = LangbookDbSchema.Tables.alphabets;
    ...
    ImmutableIntSet findCorrelationsUsedInAgents() {
        final LangbookDbSchema.AgentsTable agents = LangbookDbSchema.Tables.agents;
        final DbQuery query = new DbQuery.Builder(agents)
    ...
    @Override
    public AcceptationId findRuledAcceptationByRuleAndBaseAcceptation(int rule, AcceptationId baseAcceptation) {
        final LangbookDbSchema.RuledAcceptationsTable ruledAccs = LangbookDbSchema.Tables.ruledAcceptations;
        final LangbookDbSchema.AgentsTable agents = LangbookDbSchema.Tables.agents;
        final DbQuery query = new DbQueryBuilder(ruledAccs)
    ...
    boolean isAcceptationStaticallyInBunch(int bunch, AcceptationIdInterface acceptation) {
        final LangbookDbSchema.BunchAcceptationsTable table = LangbookDbSchema.Tables.bunchAcceptations;
        final DbQuery query = new DbQueryBuilder(table)
                .where(table.getBunchColumnIndex(), bunch)
                .where(table.getAcceptationColumnIndex(), acceptation)
    ...
    Integer findBunchSet(IntSet bunches) {
        final LangbookDbSchema.BunchSetsTable table = LangbookDbSchema.Tables.bunchSets;
        if (bunches.isEmpty()) {
    ...
    ImmutableIntPairMap findRuledConceptsByRuleInvertedMap(int rule) {
        final LangbookDbSchema.RuledConceptsTable table = LangbookDbSchema.Tables.ruledConcepts;
        final DbQuery query = new DbQuery.Builder(table)
                .where(table.getRuleColumnIndex(), rule)
                .select(table.getIdColumnIndex(), table.getConceptColumnIndex());
    ...
    Integer getRuleByRuledConcept(int ruledConcept) {
        final LangbookDbSchema.RuledConceptsTable table = LangbookDbSchema.Tables.ruledConcepts;
        final DbQuery query = new DbQuery.Builder(table)
                .where(table.getIdColumnIndex(), ruledConcept)
                .select(table.getRuleColumnIndex());

        return selectOptionalFirstIntColumn(query);
    }

    ImmutableIntSet findAgentsByRule(int rule) {
        final LangbookDbSchema.AgentsTable table = LangbookDbSchema.Tables.agents;
        final DbQuery query = new DbQuery.Builder(table)
                .where(table.getRuleColumnIndex(), rule)
                .select(table.getIdColumnIndex());
    ...
    Integer findQuizDefinition(int bunch, int setId) {
    ...
    ImmutableIntSet findAgentsWithoutSourceBunches() {
        final LangbookDbSchema.AgentsTable agents = LangbookDbSchema.Tables.agents;
        final DbQuery query = new DbQuery.Builder(agents)
    ...
    ImmutableIntSet findAffectedAgentsByAcceptationCorrelationModification(AcceptationId acceptation) {
        final LangbookDbSchema.AgentsTable agents = LangbookDbSchema.Tables.agents;
        final LangbookDbSchema.BunchSetsTable bunchSets = LangbookDbSchema.Tables.bunchSets;
        final LangbookDbSchema.BunchAcceptationsTable bunchAcceptations = LangbookDbSchema.Tables.bunchAcceptations;
    ...
    ImmutableIntSet findQuizzesByBunch(int bunch) {
        final LangbookDbSchema.QuizDefinitionsTable quizzes = LangbookDbSchema.Tables.quizDefinitions;
        final DbQuery query = new DbQuery.Builder(quizzes)
                .where(quizzes.getBunchColumnIndex(), bunch)
    ...
    Integer findQuestionFieldSet(Iterable<QuestionFieldDetails<AlphabetId>> collection) {
        ...
        try (DbResult result = _db.select(query)) {
            if (result.hasNext()) {
                ...
                foundSet.add(new QuestionFieldDetails<>(_alphabetIdSetter.getKeyFromDbValue(row.get(1)), row.get(2).toInt(), row.get(3).toInt()));
    ...
    boolean isBunchAcceptationPresentByAgent(int agentId) {
        final LangbookDbSchema.BunchAcceptationsTable table = LangbookDbSchema.Tables.bunchAcceptations;
        final DbQuery query = new DbQuery.Builder(table)
                .where(table.getAgentColumnIndex(), agentId)
                .select(table.getIdColumnIndex());
    ...
    ImmutableIntKeyMap<ImmutableIntSet> findAffectedAgentsByItsSourceWithTarget(int bunch) {
        if (bunch == 0) {
    ...
    ImmutableIntKeyMap<ImmutableIntSet> findAffectedAgentsByItsDiffWithTarget(int bunch) {
        if (bunch == 0) {
            return ImmutableIntKeyMap.empty();
        }
    ...
    private ImmutableSet<AcceptationId> readAllAcceptationsInBunch(AlphabetId alphabet, int bunch) {
        final LangbookDbSchema.BunchAcceptationsTable bunchAcceptations = LangbookDbSchema.Tables.bunchAcceptations;
        final LangbookDbSchema.StringQueriesTable strings = LangbookDbSchema.Tables.stringQueries;
        final DbQuery query = new DbQueryBuilder(bunchAcceptations)
                .join(strings, bunchAcceptations.getAcceptationColumnIndex(), strings.getDynamicAcceptationColumnIndex())
    ...
    private ImmutableSet<AcceptationId> readAllPossibleSynonymOrTranslationAcceptationsInBunch(AlphabetId alphabet, int bunch) {
        final LangbookDbSchema.AcceptationsTable acceptations = LangbookDbSchema.Tables.acceptations;
        final LangbookDbSchema.BunchAcceptationsTable bunchAcceptations = LangbookDbSchema.Tables.bunchAcceptations;
        final LangbookDbSchema.StringQueriesTable strings = LangbookDbSchema.Tables.stringQueries;
    ...
    private ImmutableSet<AcceptationId> readAllRulableAcceptations(AlphabetId alphabet, int rule) {
        final LangbookDbSchema.StringQueriesTable strings = LangbookDbSchema.Tables.stringQueries;
        final LangbookDbSchema.RuledAcceptationsTable ruledAcceptations = LangbookDbSchema.Tables.ruledAcceptations;
        final LangbookDbSchema.AgentsTable agents = LangbookDbSchema.Tables.agents;
    ...
    private ImmutableSet<AcceptationId> readAllRulableAcceptationsInBunch(AlphabetId alphabet, int rule, int bunch) {
        final LangbookDbSchema.BunchAcceptationsTable bunchAcceptations = LangbookDbSchema.Tables.bunchAcceptations;
        final LangbookDbSchema.StringQueriesTable strings = LangbookDbSchema.Tables.stringQueries;
        final LangbookDbSchema.RuledAcceptationsTable ruledAcceptations = LangbookDbSchema.Tables.ruledAcceptations;
        final LangbookDbSchema.AgentsTable agents = LangbookDbSchema.Tables.agents;
    ...
    private ImmutableSet<AcceptationId> readAllPossibleAcceptationForField(int bunch, QuestionFieldDetails<AlphabetId> field) {
        switch (field.getType()) {
            case LangbookDbSchema.QuestionFieldFlags.TYPE_SAME_ACC:
                return (bunch == NO_BUNCH)? readAllAcceptations(field.alphabet) : readAllAcceptationsInBunch(field.alphabet, bunch);

            case LangbookDbSchema.QuestionFieldFlags.TYPE_SAME_CONCEPT:
                return (bunch == NO_BUNCH)? readAllPossibleSynonymOrTranslationAcceptations(field.alphabet) :
                        readAllPossibleSynonymOrTranslationAcceptationsInBunch(field.alphabet, bunch);

            case LangbookDbSchema.QuestionFieldFlags.TYPE_APPLY_RULE:
                return (bunch == NO_BUNCH)? readAllRulableAcceptations(field.alphabet, field.rule) :
    ...
    ImmutableSet<AcceptationId> readAllPossibleAcceptations(int bunch, ImmutableSet<QuestionFieldDetails<AlphabetId>> fields) {
        final Function<QuestionFieldDetails<AlphabetId>, ImmutableSet<AcceptationId>> mapFunc = field -> readAllPossibleAcceptationForField(bunch, field);
        return fields.map(mapFunc).reduce((a, b) -> a.filter(b::contains));
    }

    ImmutableSet<AcceptationId> getAllRuledAcceptationsForAgent(int agentId) {
        final LangbookDbSchema.RuledAcceptationsTable table = LangbookDbSchema.Tables.ruledAcceptations;
        final DbQuery query = new DbQuery.Builder(table)
                .where(table.getAgentColumnIndex(), agentId)
                .select(table.getIdColumnIndex());
    ...
    ImmutableMap<AcceptationId, AcceptationId> getFilteredAgentProcessedMap(int agentId, Set<AcceptationId> acceptations) {
        final LangbookDbSchema.RuledAcceptationsTable table = LangbookDbSchema.Tables.ruledAcceptations;
        final DbQuery query = new DbQuery.Builder(table)
                .where(table.getAgentColumnIndex(), agentId)
                .select(table.getAcceptationColumnIndex(), table.getIdColumnIndex());
    ...
    ImmutableIntValueMap<SentenceSpan<AcceptationId>> getSentenceSpansWithIds(int sentenceId) {
        final LangbookDbSchema.SpanTable table = LangbookDbSchema.Tables.spans;
        final DbQuery query = new DbQuery.Builder(table)
                .where(table.getSentenceIdColumnIndex(), sentenceId)
     ...
     */
    ImmutableIntSet findSentencesBySymbolArrayId(SymbolArrayId symbolArrayId) {
        final LangbookDbSchema.SentencesTable table = LangbookDbSchema.Tables.sentences;
    ...
    SymbolArrayId getSentenceSymbolArray(int sentenceId) {
        final LangbookDbSchema.SentencesTable table = LangbookDbSchema.Tables.sentences;
        final DbQuery query = new DbQuery.Builder(table)
                .where(table.getIdColumnIndex(), sentenceId)
