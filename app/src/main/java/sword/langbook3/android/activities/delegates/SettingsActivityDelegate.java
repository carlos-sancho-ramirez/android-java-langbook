...
import sword.langbook3.android.StorageUtils;
import sword.langbook3.android.Utils;
import sword.langbook3.android.db.AlphabetId;
import sword.langbook3.android.interf.ActivityExtensions;
import sword.langbook3.android.sdb.ProgressListener;

public final class SettingsActivityDelegate<Activity extends ActivityExtensions> extends AbstractActivityDelegate<Activity> implements View.OnClickListener, ProgressListener,
    ...
    private void pickFile() {
        ...
        intent.setType("*/*");
        if (!_activity.getPackageManager().queryIntentActivities(intent, 0).isEmpty()) {
            _activity.startActivityForResult(intent, REQUEST_CODE_PICK);
        }
        else {
            selectLocation();
        }
    ...
    public void onCreate(@NonNull Activity activity, Bundle savedInstanceState) {
        ...
        final View progressPanel = activity.findViewById(R.id.progressPanel);
        progressPanel.setOnClickListener(this);

        activity.findViewById(R.id.importStreamedDatabaseButton).setOnClickListener(this);
        activity.findViewById(R.id.exportStreamedDatabaseButton).setOnClickListener(this);
        activity.findViewById(R.id.importSqliteDatabaseButton).setOnClickListener(this);
        activity.findViewById(R.id.exportSqliteDatabaseButton).setOnClickListener(this);

        _preferredAlphabetSpinner = activity.findViewById(R.id.preferredAlphabetSpinner);
    ...
    public void onActivityResult(@NonNull Activity activity, int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == android.app.Activity.RESULT_OK && requestCode == REQUEST_CODE_PICK && uri != null) {
    ...
    private boolean hasReadPermission() {
        return _activity.checkPermission(Manifest.permission.READ_EXTERNAL_STORAGE, android.os.Process.myPid(), android.os.Process.myUid()) == PackageManager.PERMISSION_GRANTED;
    }

    private boolean hasWritePermission() {
        return _activity.checkPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE, android.os.Process.myPid(), android.os.Process.myUid()) == PackageManager.PERMISSION_GRANTED;
    }

    @SuppressWarnings("NewApi")
    private void requestReadPermission() {
        _activity.requestPermissions(new String[] {Manifest.permission.READ_EXTERNAL_STORAGE}, REQUEST_CODE_ASK_PERMISSION);
    }

    @SuppressWarnings("NewApi")
    private void requestWritePermission() {
        _activity.requestPermissions(new String[] {Manifest.permission.WRITE_EXTERNAL_STORAGE}, REQUEST_CODE_ASK_PERMISSION);
    }
    ...
    public void setProgress(float progress, String message) {
        final View progressPanel = _activity.findViewById(R.id.progressPanel);

        final DbManager dbManager = DbManager.getInstance();
        final boolean inProgress = dbManager.isProcessingDatabase();
        if (inProgress) {
            final ProgressBar progressBar = _activity.findViewById(R.id.progressBar);
            final TextView progressMessage = _activity.findViewById(R.id.progressMessage);
    ...
    private void loadSqliteDatabase(Uri uri) {
        ...
        if (uri != null) {
            try {
                ...
                Utils.copyFile(_activity, uri, manager.getDatabasePath());
    ...
    private void saveSqliteDatabase(Uri uri) {
        ...
        try {
            Utils.copyFile(_activity, dbPath, uri);
        }
        catch (IOException e) {
