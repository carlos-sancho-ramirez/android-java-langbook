...
public final class AddAcceptationWordEditorController implements WordEditorActivity.Controller {

    @NonNull
    private final LanguageId _language;
    private final String _searchQuery;

    public AddAcceptationWordEditorController(
    ...
    public UpdateFieldsResult updateFields(@NonNull Activity activity, @NonNull MapGetter<ImmutablePair<AlphabetId, AlphabetId>, Conversion<AlphabetId>> conversions, ImmutableList<String> texts) {
        ...
        if (texts == null) {
            ...
            for (int fieldIndex = 0; fieldIndex < fieldCount; fieldIndex++) {
                ...
                proposedText = queryTextIsValid.contains(fieldIndex)? queryText :
    ...
    public static final Creator<AddAcceptationWordEditorController> CREATOR = new Creator<AddAcceptationWordEditorController>() {

        @Override
        public AddAcceptationWordEditorController createFromParcel(Parcel source) {
            final LanguageId language = LanguageIdParceler.read(source);
            final String searchQuery = source.readString();
            return new AddAcceptationWordEditorController(language, searchQuery);
        }

        @Override
        public AddAcceptationWordEditorController[] newArray(int size) {
            return new AddAcceptationWordEditorController[size];
        }
    };
}
